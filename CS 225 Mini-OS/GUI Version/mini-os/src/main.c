...
int main(void) { // Defines program entry point that runs the interactive SERC Mini-OS console.
	...
	do { // Uses do-while loop as required so menu is shown repeatedly until user chooses exit.
		...
		printf("7. Launch GUI Dashboard\n"); // Prints the dashboard option so the Raylib interface is discoverable from the CLI.
		printf("8. Run Live Demo Scenario\n"); // Prints top-level section 8 so presenters can execute full scripted walkthrough quickly.
		...
		switch (main_choice) { // Dispatches to selected major subsystem based on top-level menu input.
			...
			case 7: // Handles Raylib dashboard launch from the existing console interface.
			...
			case 8: // Handles automated live demo scenario execution.
